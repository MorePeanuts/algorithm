...
### Approach 2
**Method Name**: Binary Search (Left Boundary Comparison)

**Principle:**
Use the first element of the array as the reference to determine the position of the minimum value through binary search. If the middle element is greater than or equal to the left boundary, the minimum value is in the right half; otherwise, it's in the left half.

**Steps:**
1. First check if the array is not rotated (first element < last element), if so return the first element directly
2. Initialize left and right pointers `l=0`, `r=len(nums)`
3. While `l < r`, compute middle position `m = l + (r-l)/2`
4. If `nums[m] >= nums[0]`, the minimum is in the right half, adjust `l = m + 1`
5. Otherwise the minimum is in the left half (including m), adjust `r = m`
6. After the loop, check if `l` is out of bounds, if so return the last element, otherwise return `nums[l]`

**Example:**
- Input `nums = [4,5,6,7,0,1,2]`
- nums[0] = 4 > nums[6] = 2, enter binary search
- m=3, nums[3]=7 >= 4 → l=4
- m=5, nums[5]=1 < 4 → r=5
- m=4, nums[4]=0 < 4 → r=4
- l==r=4, return nums[4]=0
