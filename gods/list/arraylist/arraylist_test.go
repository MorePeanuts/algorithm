...
func TestAppend(t *testing.T) {
	...
	if largeList.Len() != 100 {
		t.Errorf("Append 100 values: length should be 100, got %d", largeList.Len())
	}
	for i := 0; i < 100; i++ {
...
func TestGrowthAndShrink(t *testing.T) {
	...
	for i := 0; i < 500; i++ {
		list.Append(i)
		if list.Len() != i+1 {
			t.Errorf("After append %d: length should be %d, got %d", i, i+1, list.Len())
		}
	}
	...
	for i := 499; i >= 0; i-- {
		list.Remove(i)
		if list.Len() != i {
			t.Errorf("After remove %d: length should be %d, got %d", i, i, list.Len())
		}
	}
