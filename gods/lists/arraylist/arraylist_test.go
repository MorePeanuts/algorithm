...
func TestAppend(t *testing.T) {
	...
	// Test append many values to trigger growth
	largeList := New[int]()
	for i := 0; i < 100; i++ {
		largeList.Append(i)
	}
	if largeList.Size() != 100 {
		t.Errorf("Append 100 values: length should be 100, got %d", largeList.Size())
	}
	for i := 0; i < 100; i++ {
		if val, ok := largeList.Get(i); !ok || val != i {
			t.Errorf("Append 100 values: index %d should be %d, got %v", i, i, val)
		}
	}
}
...
func TestRemove(t *testing.T) {
	...
	// Test remove many to trigger shrink
	largeList := New[int]()
	for i := 0; i < 100; i++ {
		largeList.Append(i)
	}
	...
	if !largeList.IsEmpty() {
		t.Error("Remove all elements should leave list empty")
	}
}
...
// Test growth and shrink behavior
func TestGrowthAndShrink(t *testing.T) {
	...
	for i := 0; i < 500; i++ {
		list.Append(i)
		if list.Size() != i+1 {
			t.Errorf("After append %d: length should be %d, got %d", i, i+1, list.Size())
		}
	}

	// Verify all elements
	for i := 0; i < 500; i++ {
		if val, ok := list.Get(i); !ok || val != i {
			t.Errorf("Index %d should be %d, got %v", i, i, val)
		}
	}

	// Remove from end to trigger shrink
	for i := 499; i >= 0; i-- {
		list.Remove(i)
		if list.Size() != i {
			t.Errorf("After remove %d: length should be %d, got %d", i, i, list.Size())
		}
	}
